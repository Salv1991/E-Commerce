<div 
    ...
    class="grid grid-cols-8 items-center justify-center gap-5 h-40 w-full p-4 bg-gray-50">
    
    <form 
        id="wishlist-form-{{ $product->id }}" 
        action="{{ route('wishlist.create', $product->id) }}" 
        method="post" 
        class="wishlist-form col-span-1 m-auto">
        @csrf
    ...
    <a href="{{route('product', $product)}}" class="col-span-1 h-full overflow-hidden ">
        <img 
